if [[ -n "${__nix_qtbase-}" ]]; then
    ...
    # In intra-ISA cross (same config string, different gcc.arch) the F4
    # strictDeps relax causes both BUILD-platform and HOST-platform qtbase setup
    # hooks to be sourced.  BUILD qtbase is needed for cmake tool resolution
    # (rcc, moc, qmlcachegen etc.) while HOST qtbase is the actual library being
    # linked.  The two share the same ABI (same config string), so the conflict
    # is a false positive.  Suppress it when NIX_IS_INTRA_ISA_CROSS=1.
    if [[ "$__nix_qtbase" != "@qtbaseOut@" && "${NIX_IS_INTRA_ISA_CROSS-}" != "1" ]]; then
    ...
    qtToolsHook() {
        if [ -f "$1/libexec/qhelpgenerator" ]; then
            if [[ -n "${qtToolsPathSeen:-}" && "${qttoolsPathSeen:-}" != "$1" && "${NIX_IS_INTRA_ISA_CROSS-}" != "1" ]]; then
                echo >&2 "Error: detected mismatched Qt dependencies:"
                echo >&2 "    $1"
