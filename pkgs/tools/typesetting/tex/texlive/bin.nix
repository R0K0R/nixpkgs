...
rec {
  ...
  core-big = stdenv.mkDerivation {
    ...
    # ICU 76 moved ucol_openRules from libicuuc to libicui18n; upmendex
    # configure only requests icu-uc and icu-io pkg-config components.
  ...
  bibtex8 = stdenv.mkDerivation {
    ...
    # ICU 76 moved ucol_strcollUTF8 from libicuuc to libicui18n; the bibtex-x
    # configure script only requests icu-uc and icu-io pkg-config components.
    env.NIX_LDFLAGS = "-licui18n";
  ...
  lib.optionalAttrs (!clisp.meta.broken) # broken on aarch64 and darwin (#20062)
    {
      ...
      xindy = stdenv.mkDerivation {
        ...
        nativeBuildInputs = [
          pkg-config
          perl
          # clisp runs during configure (--with-clisp-runtime probe) and during
          # build (generates xindy.mem).  It must be in nativeBuildInputs so it
          # lands in PATH on the BUILD machine; buildInputs only provides HOST
          # libraries and is not added to PATH under strictDeps / cross builds.
          clisp
        ];
        buildInputs = [
          libiconv
